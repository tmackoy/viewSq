...
with open(output_file, mode='w') as out_file:
    ...
    for grp_pair, atombins in atom_participation_in_groups.items():
        ...
        if not is_first_frame and grp_pair in prev_group_pairs_bins:
    ...
    for grp_pair, atombins in prev_group_pairs_bins.items():
        out_line = ''
        if not (grp_pair in atom_participation_in_groups) and not is_first_frame:
            out_file.write(grp_pair + "\n")
            for bin, value in atombins.items():
                out_line += str(str(bin) + ":" + str(value)) + ","
            out_file.write(out_line[:-1] + "\n")
